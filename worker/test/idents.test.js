...
async function seed(db) {
  for (const sql of [
    ...
    `CREATE TABLE movies (id TEXT PRIMARY KEY, "cast" TEXT, "order" INTEGER, blurb TEXT, created_at TEXT DEFAULT (${NOW}), updated_at TEXT DEFAULT (${NOW}), deleted_at TEXT)`,
...
const cols = ["id", "cast", "order", "blurb", "updated_at"];

test("push, pull and derive all work on keyword-named columns", async () => {
  const db = await seed(new D1Shim());
  const row = { id: "m1", cast: "Ada", order: 1, blurb: null, updated_at: "2026-09-04T00:00:00.000Z" };
  ...
  expect(typeof cursor.max_updated_at === "string" || typeof cursor.max_hub_at === "string").toBe(true);
});
